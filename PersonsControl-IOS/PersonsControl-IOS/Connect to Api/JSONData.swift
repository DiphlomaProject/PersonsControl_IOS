...
//  PersonsControl-IOS
//
//  Created by Валерий Мельников on 07.09.2018.
...
struct DataClass: Codable {
    let roleNames, img: JSONNull?
    let claims: [JSONAny]
    let logins: [Login]
    let roles: [Role]
    let address, country, city, displayName: String
    let email: String
    let emailConfirmed: Bool
    let passwordHash, securityStamp, phoneNumber: String
    let phoneNumberConfirmed, twoFactorEnabled: Bool
    let lockoutEndDateUTC: JSONNull?
...
struct Login: Codable {
    let loginProvider, providerKey, userID: String
